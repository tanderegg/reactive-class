...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      Promise[T]().complete(Try[T](value)).future
    }
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      fs.foreach{ _ onFailure {case fail => p.tryFailure(fail)} }
      Future.sequence(fs).foreach(p trySuccess _)
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      if(f.isCompleted) {
        ...
        throw new NoSuchElementException("Result is not ready.")
      }
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete { case x => p complete Try(cont(f)) }
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete { case x => p complete Try(cont(x)) }
      p.future
    }
